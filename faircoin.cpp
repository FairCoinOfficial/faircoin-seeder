...
#define FAIRCOIN_SEED_NONCE  0x0539a019ca550825ULL
...
//    printf("%s: SEND %s\n", ToString(you).c_str(), pszCommand); 
  ...
  void PushVersion() {
    ...
    uint64 nLocalNonce = FAIRCOIN_SEED_NONCE;
    int64 nLocalServices = 0;
    CAddress me(CService("0.0.0.0"));
    BeginMessage("version");
    int nBestHeight = GetRequireHeight();
    string ver = "/faircoin-seeder:0.01/";
    uint8_t fRelayTxs = 0;
...
/*
int main(void) {
  CService ip("seed1.fairco.in", 46372, true);
  vector<CAddress> vAddr;
  vAddr.clear();
  int ban = 0;
